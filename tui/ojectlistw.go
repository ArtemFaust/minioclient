	...
	objectlist.AddButton(&winman.Button{
		...
		OnClick: func() {
			...
			modallist := tview.NewList().
				...
				AddItem("Удалить", "", '➖', func() { modal.Hide() }).
...
func download(wm *winman.Manager, app *tview.Application, minioClient *minio.Client, bucketname string, objectkey string, ver string, logger chan string) {
	// Контекст выполнения
	ctx, cancel := context.WithCancel(context.Background())
	...
	if len(objectkey) > 0 && objectkey[len(objectkey)-1] != '/' {
		...
		processDirectory(minioClient, bucketname, objectkey, ver, logger, ch, ctx)
		pm.SetBorderColor(tcell.ColorGreen) // По окончанию выполнения поля делаем зелеными
	}
	...
	for obj := range list {
		...
		default:
			...
			if obj.Key[len(obj.Key)-1] != '/' {
				...
				processDirectory(minioClient, bucketname, obj.Key, ver, logger, ch, ctx)
			}
		}
	}
}
